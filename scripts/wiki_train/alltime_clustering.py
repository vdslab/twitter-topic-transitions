...
def main():
    ...
    for eps in eps_list:
        for min_samples in min_samples_list:
            ...
            stats = pd.Series(
                [i for i in clf.labels_ if i != -1]).value_counts().values
            ...
            pyplot.title("alltime_map with eps = " + str(eps) +
                         " min_samples = " + str(min_samples))

            few_count = []
            for i in range(1, 5):
                few_count.append({i: stats.count(i)})
            img_info_r = "n_clusters: " + \
                str(n_clusters) + ", outliners: " + \
                str(outliners) + ", biggest: " + str(stats[0])
